...
class ContinueWatchingSerializer(serializers.ModelSerializer):
    ...
    video_url = serializers.ImageField(source='episode.video_file', read_only=True)

    class Meta:
        model = EpisodeWatchProgress
        fields = [
            'id', 'episode_id', 'season_id', 'series_id', 'series_name', 'total_episodes',
            'image', 'timestamp', 'video_url',
        ]
...
class DramaSeriesHomeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'title', 'description', 'rating', 'poster_image',
                  'trailer_url', 'total_episodes', 'categories', 'tags', 'languages', 'view_count']
